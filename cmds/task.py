    ...
    def __init__(self, *args, **kwargs):
        ...
        async def interval():
            ...
            while not self.bot.is_closed():
                ...
                anime_href = (
                    anime_items[0].select_one("a.anime-card-block").get("href")
                )  # 觀看鏈結

                if anime_href != jdata["bahalink"]:  # 若網站有更新
                    for anime_item in anime_items:
                        anime_name = anime_item.select_one(
                            ".anime-name > p"
                        ).text.strip()  # 動畫名稱
                        anime_watch_number = anime_item.select_one(
                            ".anime-watch-number > p"
                        ).text.strip()  # 觀看人數
                        ...
                        anime_date = (
                            anime_item.select_one(".anime-date-info")
                            .contents[-1]
                            .string.strip()
                        )
                        anime_time = anime_item.select_one(
                            ".anime-hours"
                        ).text.strip()  # 日期與時間
                        anime_img = anime_item.select_one("img.lazyload").get(
                            "data-src"
                        )  # 動漫圖片
                        ...
                        embed.add_field(
                            name="觀看次數", value=f"{anime_watch_number}", inline=True
                        )
                        embed.add_field(
                            name="集數", value=f"{anime_episode}", inline=True
                        )
                        embed.add_field(
                            name="動畫鏈結",
                            value=f"https://ani.gamer.com.tw/{anime_href}",
                            inline=True,
                        )
                        embed.add_field(
                            name="上傳時間", value=f"{anime_date} {anime_time}", inline=True
                        )
                        ...
                        if (
                            anime_time
                            != anime_items[count + 1]
                            .select_one(".anime-hours")
                            .text.strip()
                        ):
                            count = 0
                            jdata["bahalink"] = (
                                anime_items[0]
                                .select_one("a.anime-card-block")
                                .get("href")
                            )
